...
export class CartService {
  ...
  addCartItem(cartItem: CartProductType) {
    if (!this.cartVal.cartId) {
      const newCartId = 111;
      this.cartSubject.next({ cartId: newCartId, cartTotalPrice: 0, cartProducts: [] });
      this.addCartItemLocal(cartItem, newCartId);
    }
    else
      this.addCartItemLocal(cartItem);
  }
  /**
   ...
   */
  private addCartItemLocal(cartItem: CartProductType, newCartId?: number) {
    if (newCartId)
      cartItem.cart_id = newCartId;

    const cart = this.cartVal;
    const newCart = [...cart.cartProducts];
    cartItem.cart_product_id = cart.cartProducts.length;
    console.log(cartItem.cart_product_id);
