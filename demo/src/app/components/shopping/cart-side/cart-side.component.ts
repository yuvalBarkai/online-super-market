...
export class CartSideComponent implements OnInit, OnDestroy {
 ...
 /*  saveImage(e: Event) {
   ...
   */
  editProduct() {
    if (this.adminFormFields.product_id && this.f?.valid) {
      this.ProductsService.editProductLocally(this.adminFormFields);
      this.errorMsg = "";
    }
  }
  /**
  ...
  addNewProduct() {
    if (this.f?.valid) {
      console.log(this.adminFormFields);
      this.ProductsService.addProductLocally(this.adminFormFields);
      this.errorMsg = "";
      this.f?.resetForm();
    }
  }
